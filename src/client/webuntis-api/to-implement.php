...
class WebUntis {
    ...
    static function TestNoAccLogin($school) {
        ...
        $result = self::AskUntis("https://" . self::INPUT_SERVER . "/WebUntisapi/public/timetable/weekly/pageconfig?type=1&date=2018-06-25");
    ...
    private static function Login($try = 0) {
        if (BasicTools::IsSenseful($logLastTime)) {
            BasicTools::Log($school . $user . " LoginTimeSpan:" . (time() - $logLastTime));
        }
        $logLastTime = time();
        $result = self::AskUntis(
            "https://" . self::INPUT_SERVER . "/WebUntis/j_spring_security_check",
            "school=" . urlencode(BasicTools::TestSessVar("school")) . "&j_username=" . urlencode(BasicTools::TestSessVar("user")) . "&j_password=" . urlencode(BasicTools::TestSessVar("password")) .
            "&token="
        );
            ...
            if (self::RENEW_CACHE) {
                ...
                if (is_array($lastT) && !count($lastT) == 0) {
                    if (strtotime($lastT["lastLook"]) < strtotime($lastT["CURRENT_TIMESTAMP"]) - 60) {
                        $answer = self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/public/timetable/weekly/pageconfig?type=1", null, true, true);
    ...
    static function GetGeneralGridInfo() {
        return self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/public/timegrid", null);
    }
    ...
    static function GetTimeGridInfo($date, $type, $id, $startTime = "00", $endTime = "2400", $period = "") {
        ...
        return self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/public/period/info?date=$date&starttime=$startTime&endtime=$endTime&elemid=$id&elemtype=$type&ttFmtId=1$period", null);
    }
    static function GetInfo() {
        return self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/public/info/infoWidgetData", null);
    }

    static function GetNews($date) {
        //heutiger tag (20181231)
        return self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/public/news/newsWidgetData?date=" . $date, null, true, true);
    }

    static function GetAbsence($stid, $startdate, $enddate, $excusestatus = -1) {
        return self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/classreg/absences/students?studentId=" . $stid . "&startDate=" . $startdate . "&endDate=" . $enddate . "&excuseStatusId=" .
            $excusestatus . "&includeTodaysAbsence=true", null, false, true);
    }
    static function GetConsultationRegistredations() {
        return self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/public/officehours/registrations", null, false, true);
    }
    static function GetConsultationRegInfo($period, $teacher) {
        return self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/public/officehours/registrationdata?periodId=$period&teacherId=$teacher", null, true, true);
    }
    static function ConsultationRegister($period, $teacher, $date, $start, $end, $text = "", $ttext = "") {
        return self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/public/officehours/registrations", json_encode(array("periodId" => $period, "teacherId" => $teacher, "date" => $date, "startTime" => $start, "endTime" => $end, "userText" => $text, "teacherText" => $ttext)), false, true);
    }
    static function ConsultationUnregister($period, $teacher, $text = "") {
        return self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/public/officehours/registrations?periodId=$period&teacherId=$teacher&userText=$text", null, false, true, true);
    }
    static function GetClassRoles($startdate, $enddate) {
        //heutiger tag (20181231)
        return self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/classreg/classservices?startDate=" . $startdate . "&endDate=" . $enddate, null, false);
    }

    static function GetHomeworks($startdate, $enddate) {
        //heutiger tag (20181231)
        return self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/homeworks/lessons?startDate=" . $startdate . "&endDate=" . $enddate, null, false, true);
    }

    static function GetExams($startdate, $enddate) {
        //heutiger tag (20181231)
        return self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/exams?startDate=" . $startdate . "&endDate=" . $enddate, null, false, true);
    }
    ...
    static function GetReportInfo() {
        return self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/polling/REPORT", null, false, true);
    }
    ...
    static function GetPersonalInfo() {
        ...
        $pI = self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/app/config", null, false);
    ...
    static function GetConsultationHours($date, $clientId = -1) {
        //heutiger tag (20181231)
        return self::AskFor("https://" . self::INPUT_SERVER . "/WebUntisapi/public/officehours/hours?date=" . $date . "&klasseId=" . $clientId, null);
    }
    ...
    static function AvailableRooms($start, $end) {
        ...
        return self::AskUntis("https://neilo.webuntis.com/WebUntis/jsonrpc_intern.do?school=Spengergasse&v=a4.2.9",
    ...
    static function DelAbsence($id) {
	 ...
	 static function Secret() {
        ...
        $res= self::AskUntis("https://neilo.webuntis.com/WebUntis/jsonrpc_intern.do?school=Spengergasse&v=a4.2.9",
            '{"id":"secret-test' . time() .
            '","method":"getAppSharedSecret","params":[{"password":"' . Auth::GetPassword() . '","userName":"' . $_SESSION["user"] . '","token":null}],"jsonrpc":"2.0"}');
